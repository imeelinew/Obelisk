...
它在菜单栏里保留一个很轻的原生菜单，可以快速打开书签；需要整理的时候，再进入完整的应用窗口。书签默认保存在本地 SQLite 数据库，每条记录独立使用 AES-256-GCM 认证加密。设备密钥存放在 macOS Data Protection Keychain 中，首次建库还会生成一份可离线保管的恢复密钥。查看隐藏书签和导出明文备份时，需要通过 Touch ID 或密码验证。
...
- **本地加密存储**：数据保存在 `Application Support/com.eli.Obelisk/Data/store.sqlite`；支持加密备份，并可在钥匙串记录丢失时使用恢复密钥解锁。
