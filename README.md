...
<p align="center">
  ...
  快速打开、快捷键添加、离线使用、多端同步、隐藏书签、AI 优化，以及一个漂亮的应用窗口。
</p>
...
- **菜单栏优先**：不用切换窗口，就能打开最近添加、常用分组和完整书签列表
- **原生设置窗口**：用 macOS 风格的窗口搜索、整理、隐藏、归档书签。
- **隐藏书签**：通过设备所有者认证保护敏感书签。
- **离线优先同步**：本地数据保存在 `Application Support/com.eli.Obelisk/Sync/obelisk-sync.sqlite`，网络恢复后自动上传本地操作并接收其他设备的变更。
- **AI 优化**：Obelisk 支持通过 BYOK 自动优化和翻译新书签标题，这项功能名为 Intelligence
