...
github_repo="${OBELISK_GITHUB_REPOSITORY:-imeelinew/Obelisk}"
team_id="${OBELISK_TEAM_ID:-5Q5QT76MJU}"
work_dir=$(mktemp -d /tmp/obelisk-release.XXXXXX)
...
git add Obelisk.xcodeproj/project.pbxproj
if ! git diff --cached --quiet; then
    git commit -m "Release $tag"
fi

archive_path="$work_dir/Obelisk.xcarchive"
...
app_path="$archive_path/Products/Applications/Obelisk.app"
[[ -d "$app_path" ]] || {
...
archive_file="$work_dir/Obelisk-${version}.zip"
